...
import {EditTags} from "../_auth.tags.index.tsx";
...
export function RecipesPage() {
    ...
    return (
        <div className="content-grid space-y-4">
            <FieldSet legend={"..."} className={{fieldSet: "flex-wrap"}}>
                <ButtonGroup>
                    ...
                    <Button onClick={() => navigate({
                        to: "/recipes",
                        search: (params) => ({...params, massEditTags: massEditTags === true ? undefined : true})
                    })}
            ...
            {massEditTags ? <EditRecipes token={token} recipes={recipes}/> : <ul className="grid gap-4">
                {recipes.map((recipe) => (
...
function EditRecipes(props: { token: string, recipes: Recipe[] }) {
